...
def save_to_excel_pclp(data_list):
    """Membuat file Excel (.xlsx) dengan format blok PCLP yang benar."""
    output = io.BytesIO()
    
    # Kita harus membangun list of lists manual karena strukturnya tidak rata
    rows = []
    rows.append(["GENERATED BY PCLP STUDIO", "", "", "", "", ""])
    rows.append(["", "", "", "", "", ""]) # Spacer
    ...
    for i, item in enumerate(data_list):
        ...
        xs = [p[0] for p in pts]
        ys = [p[1] for p in pts]
        min_elv = min(ys) if ys else 0
        
        # Baris Header (Line 1)
        # Format: [No, Name, "Elv. Min", "X", x1, x2...]
        # Tapi biasanya di PCLP baris 1 cuma header X
        row_1 = ["", "", "Elv. Min", "X"] + xs
        rows.append(row_1)
        
        # Baris Data (Line 2)
        # Format: [No, STA, MinElv, "Y", y1, y2...]
        row_2 = [i+1, sta, min_elv, "Y"] + ys
        rows.append(row_2)
        
        # Baris Datum (Line 3 - Optional)
        datum = math.floor(min_elv - 2)
        row_3 = ["", "", "Datum", datum]
        rows.append(row_3)
    ...
    # Buat DataFrame Pandas (Ini trik agar bisa save ke Excel)
    df = pd.DataFrame(rows)
    
    # Save ke Excel
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        df.to_excel(writer, sheet_name='DataOGL', index=False, header=False)
...
def generate_dxf_smart(results, mode="cross"):
    ...
    if mode == "long":
        ...
        msp.add_mtext("LONG SECTION PROFILE", dxfattribs={'char_height': 2.0, 'layer': 'TEXT'}).set_location(insert=(start_x, 10), attachment_point=ezdxf.const.MTEXT_TOP_LEFT)
    else:
        x_gap = 60
        count = 0
        for item in results:
...
# 4. UI STREAMLIT
# ==========================================
st.set_page_config(page_title="PCLP Studio v3.2", layout="wide")
st.title("🚜 PCLP Studio v3.2 (Final Release)")
        ...
        if f_cross:
            try:
                ...
                if st.button("RUN CROSS SECTION"):
                    d_ogl, d_des = [], []
                    if s_ogl != "(Tidak Ada)": d_ogl = parse_pclp_smart(pd.read_excel(f_cross, sheet_name=s_ogl, header=None))
                    if s_des != "(Tidak Ada)": d_des = parse_pclp_smart(pd.read_excel(f_cross, sheet_name=s_des, header=None))
                    
                    final_res = []
                    if not d_ogl: d_ogl = [{'STA': f"D_{i}", 'points': []} for i in range(len(d_des))]
                    if not d_des: d_des = [{'STA': f"D_{i}", 'points': []} for i in range(len(d_ogl))]
            ...
            except: st.error("Format File Error. Save As .xlsx dulu.")
    ...
    with col2:
        if 'res_cross' in st.session_state:
            ...
            if res:
                ...
                if item['points_desain']: ax.plot(*zip(*item['points_desain']), 'r-o', label='Desain')
...
with tab3:
    ...
    with col_g2:
        # TOMBOL GENERATE DISINI
        if f_dem and f_shp and st.button("GENERATE DATA (XLSX & DXF)"):
            if mode_gen == "Cross Section Data":
                # Generate Cross Data
                res_gis, err = generate_cross_from_gis(f_dem, f_shp, interval, w_left, w_right)
                if not err:
                    st.success(f"Berhasil! {len(res_gis)} Cross Section terbentuk.")
                    
                    # 1. Output XLSX
                    xlsx_bytes = save_to_excel_pclp(res_gis)
                    st.download_button("📥 DOWNLOAD EXCEL (.xlsx)", xlsx_bytes, "GIS_Cross_Data.xlsx", 
                                     "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
                    
                    # 2. Output DXF
                    # Transformasi format GIS ke format DXF Engine (tambah key 'points_tanah')
                    dxf_input = []
                    for item in res_gis:
                        dxf_input.append({
                            'STA': item['STA'],
                            'cut': 0, 'fill': 0,
                            'points_tanah': item['points'],
                            'points_desain': [] # Desain kosong dulu
                        })
                    
                    dxf_bytes = generate_dxf_smart(dxf_input, mode="cross")
                    st.download_button("📥 DOWNLOAD AUTOCAD (.dxf)", dxf_bytes, "GIS_Cross_Draw.dxf", "application/dxf")
                    
                    st.info("Info: DXF ini hanya berisi Garis Tanah Asli. Untuk menghitung Cut/Fill, silakan upload ulang file Excel hasil generate ini ke Tab 1 (Cross Section).")
                else:
                ...
                if not err:
                    ...
                    st.download_button("📥 DOWNLOAD CSV", df_long.to_csv(index=False).encode(), "GIS_Long_Data.csv", "text/csv")
                    
                    # DXF Long Section
                    # Format: (List Tanah, List Desain)
                    mt = [(r[0], r[1]) for r in long_pts] # Convert DF to List of Tuples
                    md = []
                    dxf_bytes = generate_dxf_smart((mt, md), mode="long")
                    st.download_button("📥 DOWNLOAD DXF", dxf_bytes, "GIS_Long_Draw.dxf", "application/dxf")
