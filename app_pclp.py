...
# Konfigurasi Halaman
st.set_page_config(page_title="PCLP Studio Pro v6.0", layout="wide", page_icon="🚜")
...
def hitung_cut_fill(tanah_pts, desain_pts):
    ...
    try:
        ...
        # Validasi logika sipil: 
        # Intersection = Bagian desain yang 'tertanam' di tanah (Galian/Cut)
        # Difference = Bagian desain yang 'mengambang' di atas tanah (Timbunan/Fill)
        
        # Koreksi jika terbalik (tergantung definisi poligon)
        # Biasanya: Cut = Intersection (Tanah & Desain), Fill = Desain - Tanah
    except:
...
def render_peta_situasi(dem_file, shp_file):
    ...
    try:
        ...
        with rasterio.open(dem_file) as src:
            ...
            # Buat Grid Koordinat untuk Kontur
            ...
            for dist in np.arange(0, length, 50):
                ...
                # Garis Potongan Melintang (Imajiner)
                # Hitung vektor tegak lurus untuk visualisasi arah potongan
                if dist + 1 < length:
                    pt_next = line.interpolate(dist + 1)
                    dx, dy = pt_next.x - pt.x, pt_next.y - pt.y
                    # Putar 90 derajat
                    perp_dx, perp_dy = -dy, dx
                    # Normalisasi dan scale (misal lebar 20m)
...
with tabs[0]:
        ...
        if f_upload:
            ...
            if st.button("🚀 PROSES DATA CROSS"):
                with st.spinner("Sedang memproses..."):
                    ...
                    # Gabungkan Data (Matching by Index sementara, idealnya by STA name)
                    final_data = []
                    max_len = max(len(data_ogl), len(data_dsn))
                    
                    for i in range(max_len):
                        # Ambil data tanah jika ada
    ...
    with col_view:
        if 'data_cross' in st.session_state:
            ...
            # Menggunakan select_slider agar sinkron dengan STA
            list_sta = [d['STA'] for d in data]
            selected_sta = st.select_slider("Pilih Station:", options=list_sta)
            
            # Cari index dari sta yg dipilih
            idx = list_sta.index(selected_sta)
            item = data[idx]
            
            # Kontrol Offset (Jika data tidak berpotongan)
            st.info("💡 Jika gambar tidak berpotongan, gunakan geser X/Y di bawah ini:")
            c1, c2 = st.columns(2)
            ...
            # Hitung Ulang Cut Fill dengan Offset visual
            ...
            d1.download_button("📥 Download Semua DXF", dxf_data, "Cross_Sections.dxf", "application/dxf")
            
            # Download Excel
            xlsx_data = generate_excel_report(data)
            d2.download_button("📥 Download Laporan Volume (.xlsx)", xlsx_data, "Volume_Report.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

# --- TAB 2: LONG SECTION ---
with tabs[1]:
    st.subheader("Long Section (Profil Memanjang)")
    f_long = st.file_uploader("Upload Excel Long Section", key='long_up')
    
    if f_long:
        xls_l = pd.ExcelFile(f_long)
        s_long_ogl = st.selectbox("Sheet Tanah (Long)", ["[Pilih]"] + xls_l.sheet_names)
        s_long_dsn = st.selectbox("Sheet Desain (Long)", ["[Pilih]"] + xls_l.sheet_names)
        
        if st.button("RUN LONG SECTION"):
            # Simple parser untuk long section (kolom Jarak, Elevasi)
            # Asumsi data di kolom A dan B, atau user menyesuaikan
            pts_ogl, pts_dsn = [], []
            
            try:
                if s_long_ogl != "[Pilih]":
                    df = pd.read_excel(f_long, sheet_name=s_long_ogl)
                    # Ambil 2 kolom pertama yang numerik
                ...
                st.error(f"Gagal parse long section: {e}")
...
with tabs[2]:
    ...
    with c_gis1:
        ...
        # Helper untuk SHP (perlu upload semua komponen)
        shp_file = None
        if up_shp:
            # Cari file .shp atau .geojson
            for f in up_shp:
        ...
        if 'gis_files' in st.session_state:
            ...
            with st.spinner("Merender Kontur dan Trase..."):
                ...
                if fig:
                    ...
                    if not HAS_GEO_LIBS:
                        st.warning("Tips: Pastikan install `geopandas` dan `rasterio` di environment Anda.")
